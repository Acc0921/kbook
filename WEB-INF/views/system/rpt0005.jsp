...
<body>
	<div style="high:20px;margin-top:20px;margin-bottom:20px;text-align:center;font-size:1.2em;">收入报表（时间）</div>

<div>
	
	<form id="reportForm" name="reportForm" action="${ctx}/system/rpt0005" method="get" align="center">
			...
			<div >
				...
				<table>
					<tr>
						...
						<td width="120"><fmt:formatNumber value="${rptData.revenue}" type="currency" pattern="#0.00元"/></td>
					</tr>
				</table>
				</div>
			</div>
	...
	function show(bFlag) {
		...
		function readReport(init) {
			...
			$.ajax({
				type : "get",//使用get方法访问后台
				dataType : "json",//返回json格式的数据
				url : "${ctx}/system/rpt0005/report",//要访问的后台地址
				beforeSend : function(xhr) {
					xhr.setRequestHeader("X-Custom-Header", "noUseSession");
				},
				data : {
				...
				//complete :function(){$("#load").hide();},//AJAX请求完成时隐藏loading提示
				success : function(data) {//msg为返回的数据，在这里做数据绑定
					initReport(data.categories, data.datas1, data.datas2);
				},
				error : function(xhr, textStatus, errorThrow) {
					alert('error:' + errorThrow);
				}
		...
		function initReport(categories, datas1, datas2) {
			$('#tips').hide();
			$('#container').show();
		...
		$(document).ready(function() {
			...
			$('#period').change(function() {
				var period = $(this).val();
				location.href = '${ctx}/system/rpt0005?period=' + period;
				//readReport();
			});
