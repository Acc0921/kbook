...
<body>
	<div style="high:20px;margin-top:20px;margin-bottom:20px;text-align:center;font-size:1.2em;">会员统计报表</div>

<div>
	
	<form id="reportForm" name="reportForm" action="${ctx}/system/rpt0006" method="get" align="center">
			...
			<div >
				...
				<table>
					<tr>
						...
						<td width="120"><fmt:formatNumber value="${rptData.numofmembers}" type="number" pattern="#0个"/></td>
					</tr>
				</table>
				</div>
			</div>
...
<div id="tips" align="center" style="margin-top:50px;letter-spacing:4px;font-size:1.2em;">请选择报表查询条件</div>
	...
	function init(){
		var hary='${hcSeries}'.replace(/null/g,'0').split(',');
		var totalmembers  = (Math.max.apply(Math,hary)==0)?'':Math.max.apply(Math,hary);
	    ...
	    $('#container').highcharts({
	        ...
	        subtitle: {
	        	useHTML:true,
	            text: '<p>汇报时期: ${reportDating}  &nbsp&nbsp&nbsp&nbsp 会员总量:'+totalmembers +"</p>",
	            align: 'center'
	        },
	        xAxis: {
